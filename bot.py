...
@app.on_message(filters.channel & (filters.video | filters.document) & ~filters.edited)
...
async def start_forward_cmd(client, message):
    ...
    try:
        ...
        async with app.pool.acquire() as conn:
            ...
            if fwd_limit:
                movies_to_forward = await conn.fetch("SELECT * FROM movies WHERE source_chat_id = $1 LIMIT $2", source_chat.id, fwd_limit)
            else:
                movies_to_forward = await conn.fetch("SELECT * FROM movies WHERE source_chat_id = $1", source_chat.id)
