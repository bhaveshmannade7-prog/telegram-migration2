...
async def resolve_chat_id(client: Client, ref: str | int):
    ...
    try:
        ...
        raise RuntimeError("❌ Peer not mila. Check karein ki User Account uss chat mein hai.")
...
async def start_cmd(_, message):
    await message.reply(
        ...
        "1. `/sync_channels <source_id> <target_id>` - Dono channels ko scan karke JSON file banata hai. (Zaroori)\n"
...
async def start_forward_cmd(client, message):
    ...
    fwd_limit = None
    if len(args) == 3:
        try: fwd_limit = int(args[2].strip())
        except: return await message.reply("❌ Limit number hona chahiye.")
    ...
    for unique_id, data in source_movies.items():
        ...
        try:
            ...
            # Bot token par floodwait bohot kam aata hai, par fir bhi safety rakhte hain
...
async def batch_editor(client, message, chat_ref, edit_function):
    ...
    try:
        # Bot sirf apne messages ko edit kar sakta hai, isliye history ki zaroorat nahi
        # Hum channel ko scan karenge
