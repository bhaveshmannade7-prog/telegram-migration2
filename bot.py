    ...
    async with batch_job_lock:
        try:
            ...
            await bot.edit_message_text(
                f"❌ Job Failed: {e}",
                chat_id=status_msg.chat.id, message_id=status_msg.message_id,
                parse_mode=None # Plain text error
            )
        else:
            await bot.edit_message_text(
                f"✅ Forwarding Done!\n\n"
...
async def run_the_index_job():
    ...
    try:
        ...
        # Yeh message ab plain text mein bheja jayega, isliye formatting safe hai
...
async def run_index_job_for_telebot(call):
    ...
    try:
        ...
        # Pata lagayein ki message success hai ya error
        # Success messages '✅' se shuru hote hain
        ...
        await bot.edit_message_text(
            ...
            # Agar success hai toh Markdown use karo, varna PLAIN TEXT
        ...
        if status_msg:
            # Yeh bhi ek error hai, isse hamesha PLAIN TEXT mein bhejo
            await bot.edit_message_text(
                f"❌ Job failed: {e}",
                chat_id=status_msg.chat.id, 
                message_id=status_msg.message_id, 
