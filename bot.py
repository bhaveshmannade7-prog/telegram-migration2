            ...
            if new_caption != current_caption:
                try:
                    ...
                    if "Message can't be edited" in str(e):
                        ...
                        # === FIX: new_msg_id_obj.message_id pass karein ===
...
async def upload_db_receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if not update.message.document or not update.message.document.file_name.endswith('.json'):
            await update.message.reply_text("❌ Ye JSON file nahi hai. Kripya `.json` file bhejein.")
            return WAITING_FOR_UPLOAD_FILE
...
upload_db_conv = ConversationHandler(
    ...
    states={
        WAITING_FOR_UPLOAD_FILE: [MessageHandler(filters.Document.JSON, upload_db_receive)]
    },
    fallbacks=[CommandHandler("cancel", cancel_conv)]
)
