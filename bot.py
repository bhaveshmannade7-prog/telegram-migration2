...
# === 1. FIX: ParseMode को इम्पोर्ट करें ===
...
def add_watermark_to_caption(caption):
    ...
    if watermark in caption:
        return caption
    if caption:
        # Using \n for newline is robust and avoids syntax errors
        return f"{caption}\n\n{watermark}"
    else:
        return watermark
...
async def handle_edit_error(e, update_or_status_msg):
    ...
    if "Message can't be edited" in error_text:
        # Building multiline strings safely using a list
        msg_lines = [
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = "\n".join([
        ...
        "",
        f"Ye bot aapke Telegram channel {CHANNEL_USERNAME} ko manage karta hai.",
        "",
        "/help \- Sabhi commands ki list dekho\."
    ])
    # === 2. FIX: Use MarkdownV2 ===
    await update.message.reply_text(welcome_text, parse_mode=ParseMode.MARKDOWN_V2)

async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # === 2. FIX: Escape all special chars for MarkdownV2 ===
    help_text = "\n".join([
        "📚 *Bot Commands Guide:*",
        "",
        "*1\. Indexing* 🔄",
        ...
        "/upload\\_db \- JSON backup upload karke merge karo",
...
async def check_permissions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        response_lines = [
            f"📋 *Bot Permission Check for {CHANNEL_USERNAME}*",
            "",
        ...
        if status == "administrator":
            response_lines.append(f"• `can_post_messages`: *{can_post}* (Zaroori hai)")
            response_lines.append(f"• `can_edit_messages`: *{can_edit}* (Zaroori hai)")
            response_lines.append(f"• `can_delete_messages`: *{can_delete}* (Zaroori hai)")
            response_lines.append("")
            
            if can_edit and can_delete and can_post:
                response_lines.append("✅ *Sabhi zaroori permissions sahi hain\!*")
            else:
                response_lines.append("❌ *ERROR\!* Bot ko 'Post', 'Edit' aur 'Delete' messages ki permission dein\.")
        else:
            response_lines.append("❌ *CRITICAL ERROR\!* Bot channel mein Admin hi nahi hai\.")
        
        await update.message.reply_text("\n".join(response_lines), parse_mode=ParseMode.MARKDOWN_V2)
    except Exception as e:
...
async def sync_channel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not context.user_data.get('stop_task'):
            ...
            await status_msg.edit_text(final_text, reply_markup=None, parse_mode=ParseMode.MARKDOWN_V2)
...
async def set_watermark_receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    watermark_config = db.get_watermark()
    response_text = "\n".join([
        "✅ Watermark Successfully Set!",
        "",
        "Watermark Text:",
        f"{watermark_config['text']}",
        "",
...
async def view_watermark(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status_text = "\n".join([
        ...
        "",
        f"Status: {'✅ Enabled' if watermark_config.get('enabled') else '❌ Disabled'}",
        "",
        "*Watermark Text:*",
        f"{watermark_config.get('text', 'N/A')}",
        "",
        f"*Protected Usernames:* {', '.join(watermark_config.get('usernames', [])) or 'None'}",
        f"*Protected Links:* {', '.join(watermark_config.get('links', [])) or 'None'}"
    ])
...
async def forward_movies(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await is_admin(update.effective_user.id):
        ...
        return
    if not context.args:
        await update.message.reply_text("⚠️ Usage: `/forward <channel\\_id>`", parse_mode=ParseMode.MARKDOWN_V2)
        return
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "delete_duplicates":
        ...
        deleted = 0
        status_msg = await query.edit_text("🗑️ Duplicates delete ho rahe hain...")
            ...
            for idx, movie in reversed(duplicates):
                try:
                    ...
                    if "Message to delete not found" in str(e):
                        ...
                        await status_msg.edit_text("❌ *PERMISSION ERROR\!*\nBot ko *'Delete messages'* ki permission dein\.", parse_mode=ParseMode.MARKDOWN_V2)
                        db.save_database()
                        release_lock('duplicates')
                        return
            db.save_database()
            await status_msg.edit_text(f"✅ *{deleted} Duplicate Movies Deleted\!*", parse_mode=ParseMode.MARKDOWN_V2)
        finally:
            release_lock('duplicates')
...
async def show_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    stats_text = "\n".join([
        ...
        f"✨ Captions Cleaned: {stats['total_cleaned']}",
        f"📤 Movies Forwarded: {stats['total_forwarded']}",
        f"🗑️ Duplicates Removed: {stats['duplicates_removed']}",
        f"🏷️ Watermarks Added: {stats['watermarks_added']}",
        "",
        f"Watermark Status: {'✅ Enabled' if watermark_config.get('enabled') else '❌ Disabled'}",
...
async def find_movie(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not query:
        await update.message.reply_text("⚠️ Usage: `/find <movie\\_name>`", parse_mode=ParseMode.MARKDOWN_V2)
        return
    ...
    response_lines = [f"🔍 *Search Results for '{query}' ({len(results)} found):*", ""]
    ...
    for i, movie in enumerate(results[:20]):
        ...
        # Escape special characters for MarkdownV2 in the preview
        caption_preview_escaped = re.sub(r"([_*\[\]()~`>#+-=|{}.!])", r"\\\1", caption_preview)
        
        response_lines.append(f"{i+1}\. [Link to Movie]({channel_link_base}{movie['message_id']})")
        response_lines.append(f"   `{caption_preview_escaped[:70]}...`")

    if len(results) > 20:
        response_lines.append(f"\n\.\.\.aur {len(results) - 20} results\.")

    await update.message.reply_text("\n".join(response_lines), parse_mode=ParseMode.MARKDOWN_V2, disable_web_page_preview=True)
...
async def delete_movie(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("⚠️ Usage: `/delete <message\\_id>`", parse_mode=ParseMode.MARKDOWN_V2)
        return
