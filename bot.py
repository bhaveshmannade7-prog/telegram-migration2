...
import re
import os
import asyncpg  # <-- यह Neon Tech (PostgreSQL) के लिए है
from aiohttp import web  # <-- यह Render के लिए वेब सर्वर है
...
DATABASE_URL = os.environ.get("DATABASE_URL")  # <-- ⚠️ यहाँ हम Neon Tech का URL डालेंगे
ADMIN_ID = 7263519581  # <-- आपकी एडमिन आईडी
...
async def init_db():
    """PostgreSQL डेटाबेस से कनेक्ट करता है और टेबल बनाता है।"""
    global db_pool
    if not DATABASE_URL:
        print("!! ज़रूरी: DATABASE_URL नहीं मिला! डेटाबेस काम नहीं करेगा। !!")
        return
    try:
        # Neon Tech SSL/TLS (सुरक्षित कनेक्शन) मांग सकता है
        # इसलिए हम ssl='require' का इस्तेमाल कर सकते हैं
        ...
        print("✅ PostgreSQL (Neon Tech) डेटाबेस सफलतापूर्वक कनेक्ट और सेटअप हो गया है।")
    except Exception as e:
        # अगर 'require' से एरर आए, तो बिना ssl के ट्राई करें
        try:
            ...
            print("✅ PostgreSQL (Neon Tech) डेटाबेस (बिना SSL) सफलतापूर्वक कनेक्ट हो गया है।")
        except Exception as e2:
            print(f"❌ डेटाबेस कनेक्शन एरर: {e2}")
            print("--- कृपया Neon Tech का Connection String चेक करें ---")
...
# --- 2. मेन मेन्यू और बटन्स (सिर्फ एडमिन के लिए) ---
def get_main_menu():
    buttons = [
...
# --- 3. बटन के काम (Callback Query Handler - सिर्फ एडमिन) ---
...
async def handle_callbacks(client: Client, query: CallbackQuery):
    ...
    try:
        if data == "start_forward":
            ...
            await query.message.reply("✅ **डुप्लीकेट डिलीशन शुरू!** (यह 1200+ मूवी के लिए समय ले सकता है)")
...
# मूवी फॉरवर्ड करना (PostgreSQL के साथ)
...
# डुप्लीकेट डिलीट करना (पूरी हिस्ट्री के लिए)
async def run_delete_dupes_job(message: Message):
    ...
    try:
        ...
        # [FIX] पूरी हिस्ट्री स्कैन करने के लिए `total_messages` काउंट करते हैं
        total_messages = await app.get_chat_history_count(SOURCE_CHANNEL_ID)
        await message.reply(f"🔍 चैनल में `{total_messages}` मैसेज हैं। स्कैनिंग शुरू...")
...
# कैप्शन में Footer ऐड करना (पूरी हिस्ट्री के लिए)
...
# पुराने कैप्शन क्लीन करना (पूरी हिस्ट्री के लिए)
...
if __name__ == "__main__":
    if not BOT_TOKEN:
        ...
        print("!! ज़रूरी: DATABASE_URL नहीं मिला !! (Neon Tech का URL डालें)")
    else:
        try:
            asyncio.run(main())
