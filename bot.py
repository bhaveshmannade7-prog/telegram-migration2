...
from telegram.error import BadRequest, FloodControl
...
# === NEW: Batch & Flood Control Config ===
BATCH_SIZE = 100
...
class MovieDatabase:
    ...
    def update_movie_entry(self, old_message_id, new_message, new_caption):
        ...
        try:
            for movie in self.data["movies"]:
                if movie["message_id"] == old_message_id:
                    movie["message_id"] = new_message.message_id
                    movie["caption"] = new_caption
                    if new_message.video:
                        movie["file_id"] = new_message.video.file_id
                    elif new_message.document:
                        movie["file_id"] = new_message.document.file_id
    ...
    def update_stats(self, stat_type, increment=1):
        ...
        self.data["stats"]["last_updated"] = datetime.now().isoformat()
        # Ab save_database() har function ke end mein call hoga
...
async def handle_edit_error(e, update_or_status_msg):
    ...
    if "Message can't be edited" in error_text:
        msg = (
            ...
            "1. Bot ke paas **'Edit messages'** ki permission na ho.\n"
            "2. Message 48 ghante se **purana** ho.\n"
            "3. Aap kisi **doosre Admin** ya **Channel ke naam** se kiye gaye post ko edit kar rahe hon.\n\n"
            "**Bot ab copy-mode istemal karega.**"
        )
...
async def set_watermark_receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"✅ **Watermark Successfully Set!**", parse_mode='Markdown')
    return ConversationHandler.END
...
async def view_watermark(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not watermark_config["enabled"]:
        ...
        return
    status_text = f"🏷️ **Current Watermark**\n\n**Status:** {'✅ Enabled'}\n**Text:**\n{watermark_config['text']}"
            ...
            if new_caption != current_caption:
                try:
                    ...
                    if "Message can't be edited" in str(e):
                        ...
                        db.update_movie_entry(movie["message_id"], new_msg, new_caption)
                    else:
                        raise e
                added += 1
        ...
        for i, movie in enumerate(movies):
            ...
            try:
                ...
                if "Flood control exceeded" in str(e):
                    raise e # Raise flood error to outer try-except
                continue
...
async def handle_new_movies(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.channel_post and update.channel_post.chat.id == CHANNEL_ID:
        ...
        if message.video or message.document:
            ...
            # Auto-watermark logic (copy-on-fail ke saath)
            watermark_config = db.get_watermark()
                    ...
                    try:
                        ...
                        if "Message can't be edited" in str(e):
                            ...
                            try:
                                ...
                                message = new_msg
                                caption = new_caption
                                file_id = new_msg.video.file_id if new_msg.video else new_msg.document.file_id
            ...
            for movie in db.get_all_movies():
                if movie.get("file_hash") == file_hash:
                    is_duplicate = True
                    break
            ...
            if not is_duplicate:
                ...
                print(f"New movie processed and indexed: {message.message_id}")
            else:
                print(f"Duplicate movie ignored: {message.message_id}")
...
ptb.add_handler(CommandHandler("stop", stop_task_command)) # NEW
