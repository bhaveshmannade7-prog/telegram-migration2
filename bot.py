...
async def run_forwarding_job(message, source_chat_id):
    ...
    async with batch_job_lock:
        try:
            ...
            # FIX: Error message ko plain text aur safe banaya
            error_text = str(e)
            safe_error_text = error_text.replace("[", "").replace("]", "")
            await bot.edit_message_text(
                f"❌ Job Failed:\n{safe_error_text}",
...
async def web_server():
    ...
    # TYPO FIX: 'WEBOK_PORT' ko 'WEBHOOK_PORT' kar diya gaya hai
...
if __name__ == "__main__":
    try:
        ...
        print("\nBot stopping...")
        # In a webhook setup, app.stop() might not be reachable
