...
async def sync_channel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not context.user_data.get('stop_task'):
            ...
            await status_msg.edit_text(final_text, reply_markup=None, parse_mode='Markdown')
...
async def find_movie(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    channel_link_base = f"httpsT://t.me/c/{str(CHANNEL_ID).replace('-100', '')}/"

    for i, movie in enumerate(results[:20]):
        # This was the line that caused the SyntaxError
        ...
        response_lines.append(f"   `{caption_preview[:70]}...`")
