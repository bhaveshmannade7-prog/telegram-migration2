...
class MovieDatabase:
    ...
    def update_movie_entry(self, old_message_id, new_message, new_caption):
        """Updates a movie entry after it has been copied and replaced."""
        try:
...
async def handle_edit_error(e, update_or_status_msg):
    ...
    if "Message can't be edited" in error_text:
        msg = (
            ...
            "**Kripya bot ko channel mein 'Post' aur 'Delete' permissions dekar rakhein.**"
        )
...
async def add_watermark_to_movies(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for movie in movies[:]: # Iterate over a copy
        try:
            ...
            if new_caption != current_caption:
                try:
                    ...
                    if "Message can't be edited" in str(e):
                        # 2. Agar EDIT fail ho, to COPY + REPOST karo
                        ...
                        # 3. Purana message DELETE karo
                        await context.bot.delete_message(
                            chat_id=CHANNEL_ID,
                            message_id=movie["message_id"]
                        )
                        # 4. Database mein NAYI ID update karo
                        db.update_movie_entry(movie["message_id"], new_msg, new_caption)
                    else:
                        raise e # Koi aur error hai, use raise karo
...
async def handle_new_movies(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.channel_post and update.channel_post.chat.id == CHANNEL_ID:
        ...
        if message.video or message.document:
                    ...
                    try:
                        ...
                        if "Message can't be edited" in str(e):
                            ...
                            try:
                                ...
                                # 4. Naye message ki details save karo
                                message = new_msg
                                caption = new_caption
            ...
            # Final details ko DB mein save karo
