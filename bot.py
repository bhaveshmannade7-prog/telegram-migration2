...
async def resolve_chat_id(client: Client, ref: str | int):
    ...
    client_name = "Bot Account" if client.me.is_bot else "User Account (String)"
    ...
    if isinstance(ref, int) or (isinstance(ref, str) and ref.lstrip("-").isdigit()):
        try:
            ...
            return chat
             ...
             raise RuntimeError(f"❌ Peer not mila. Check karein ki aapka **{client_name}** uss chat mein hai. Agar User Account hai, toh pehle `/sync_user_account` chalaayein.")
        except Exception as e:
            pass # Fallback
...
async def scan_channel(client: Client, chat_ref: str, filename: str, status_message):
    ...
    try:
        ...
        await status_message.edit(f"❌ Error (User Client): {e}")
        return
    ...
    try:
        ...
        await status_message.edit(f"❌ (User Client) Scan Error: `{e}`")
...
async def start_cmd(_, message):
    await message.reply(
        ...
        "1. `/sync_user_account` - (Ek baar chalaayein) User Account ke cache ko sync karta hai.\n"
        "2. `/sync_channels <source_id> <target_id>` - Dono channels ko scan karke JSON file banata hai. (Zaroori)\n"
        "3. `/start_forward <target_id>` - Bot tezi se forwarding shuru karega.\n\n"
        ...
        "* `/sync_user_account` - **(Naya)** User Account ki chat list ko refresh karta hai.\n"
...
async def sync_user_account_cmd(client, message):
    ...
    try:
        ...
        await app_user.stop()
        await status.edit(f"🎉 **User Account Sync Complete!**\nFound {count} chats. Ab numeric IDs kaam karne chahiye.\nUser-client stop ho gaya hai.")
    except Exception as e:
        await status.edit(f"❌ Main Bot Error: {e}")
        try: await app_user.stop()
        except: pass
# --------------------
...
async def index_cmd(client, message):
    ...
    try:
        ...
        await status.edit(f"❌ Main Bot Error: {e}")
        try: await app_user.stop() # Ensure user client stops on error
        except: pass
...
async def sync_channels_cmd(client, message):
    ...
    try:
        ...
        await scan_channel(app_user, source_ref, MOVIE_INDEX_DB_FILE, status)
...
async def clear_index_cmd(_, message):
    ...
    if os.path.exists(TARGET_INDEX_DB_FILE):
        ...
        count += 1
    await message.reply(f"✅ `{count}` JSON index files delete kar di hain.")
...
async def start_forward_cmd(client, message):
    ...
    if len(args) == 3:
        try: 
            ...
            return await message.reply("❌ Limit number hona chahiye.")

    # Files check
    if not os.path.exists(MOVIE_INDEX_DB_FILE):
        return await message.reply(f"❌ `{MOVIE_INDEX_DB_FILE}` nahi mili. Pehle `/index` ya `/sync_channels` chalao.")
    if not os.path.exists(TARGET_INDEX_DB_FILE):
        return await message.reply(f"❌ `{TARGET_INDEX_DB_FILE}` nahi mili. Pehle `/index_target` ya `/sync_channels` chalao.")
    ...
    try:
        ...
        with open(MOVIE_INDEX_DB_FILE, "r") as f:
            ...
            source_chat_id = source_data["chat_id"]
        ...
        return await status.edit(f"❌ Error loading DBs ya resolving chat: {e}\n\n**Hint:** Kya aapka **Bot Account** (`{client.me.username}`) target channel (`{target_ref}`) ka member hai?")
    ...
    await status.edit(
        ...
        f"Source (Cache): `{source_data['chat_name']}`\n"
...
async def batch_editor(client, message, chat_ref, edit_function):
    ...
    try:
        ...
        return await message.reply(f"❌ Error (Bot Client): {e}\n\n**Hint:** Kya aapka **Bot Account** (`{client.me.username}`) channel ka member hai?")

    status = await message.reply(f"⏳ Batch editing `{chat.title}` mein shuru kar raha hoon... (Batch 100)")
    is_editing = True
    processed_count = 0
    edited_count = 0
    ...
    try:
        ...
        await status.edit(f"❌ Editing Error: {e}")
...
async def add_caption_cmd(client, message):
    ...
    await message.reply(f"✅ Caption text '{text_to_add}' ko 'lock' kar diya hai. Ab add kar raha hoon...")
...
async def replace_caption_cmd(client, message):
    ...
    if not old_text:
        return await message.reply(f"❌ Koi 'locked' text nahi mila. Pehle `/add_caption` chalao.")

    # Naye text ko "lock" kar do
    with open(CAPTION_LOCK_FILE, "w", encoding="utf-8") as f:
        f.write(new_text)
        
    await message.reply(f"✅ Caption '{old_text}' ko '{new_text}' se replace kar raha hoon...")
