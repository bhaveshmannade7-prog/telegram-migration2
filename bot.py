...
# --- Pyrogram Imports ---
from pyrogram import Client, filters, enums # <-- enums ko import kiya
...
# =================================================================
def escape_md(text):
    """Telegram MarkdownV2 characters ko escape karta hai."""
...
async def whoami_cmd(message):
    ...
    try:
        # 'app' (Pyrogram client) se 'me' (khud) ki details fetch karo
        async with app: # Ensure app is running for this single request
            me = await app.get_me() 
        ...
        # Error ko escape karke bhejo
        await bot.edit_message_text(
            escape_md(f"❌ /whoami command fail ho gaya: {e}"),
...
async def run_forwarding_job(message, source_chat_id):
    ...
    async with batch_job_lock:
        try:
            ...
            async with app: # Ensure app is running
            ...
            # FIX: Error message ko escape kiya
            await bot.edit_message_text(
                escape_md(f"❌ Job Failed: {e}"), # <-- FIX
                chat_id=status_msg.chat.id, message_id=status_msg.message_id,
                parse_mode="MarkdownV2" # Hum escaped text bhej rahe hain
            )
        else:
            await bot.edit_message_text(
                f"✅ Forwarding Done!\n\n"
...
async def process_post(msg):
    # This runs inside the 'app' context, no need to wrap
    if not db_pool:
        return
    file_uid = get_file_id(msg)
    if not file_uid:
...
async def run_the_index_job():
    ...
    async with app: # Ensure app is running
        ...
        try:
            ...
            # Yeh message ab escape ho jayega, isliye safe hai
        ...
        async with batch_job_lock:
            ...
            # Yeh success message hai, isme Markdown safe hai
            return f"✅ Indexing Done. Added: `{count_new}` new movies.", count_new

# =================================================================
# === YAHAN HAIN ASLI TELEBOT FIXES ===
# =================================================================
async def run_index_job_for_telebot(call):
    ...
    try:
        ...
        if is_success:
            # Success message safe hai, default Markdown use karo
            await bot.edit_message_text(
                result_msg, 
                chat_id=status_msg.chat.id, 
                message_id=status_msg.message_id
            ...
            # Error message hai! Escape karo!
            await bot.edit_message_text(
                escape_md(result_msg), # <-- FIX
                chat_id=status_msg.chat.id, 
                message_id=status_msg.message_id,
                parse_mode='MarkdownV2' # Hum escaped text bhej rahe hain
            )
        ...
        if status_msg:
            # Yeh bhi ek error hai, isse bhi escape karo!
            await bot.edit_message_text(
                escape_md(f"❌ Job failed: {e}"), # <-- FIX
                chat_id=status_msg.chat.id, 
                message_id=status_msg.message_id, 
...
# =================================================================
# === TELEBOT FIXES END HERE ===
# =================================================================

# =================================================================
# === YAHAN HAIN ASLI PYROGRAM FIXES ===
# =================================================================
...
async def full_index(client, message):
    ...
    await status.edit(
        result_msg,
        # Pyrogram ke liye sahi tareeka
        parse_mode=enums.ParseMode.MARKDOWN if is_success else enums.ParseMode.DISABLED # <-- FIX
    )
# =================================================================
# === PYROGRAM FIXES END HERE ===
# =================================================================
...
# URL pehle hi fix ho chuka tha, yeh sahi hai
...
async def main():
    ...
    # 2. Pyrogram Client (app) ko start/stop karna manage karenge
    # Hum 'main' mein app.start() nahi karenge.
    # Uski jagah, har job (like run_the_index_job) `async with app:` block istemaal karega.
    # Pyrogram handlers (new_post) ke liye app.start() zaroori hai.
    ...
    if webhook_set:
        ...
        # Ab hum `exit(1)` nahi karenge, taaki web server phir bhi chale
        # Lekin Telebot kaam nahi karega
        print("Warning: Telebot will not work.")

    # 4. Web Server shuru karein (Yeh 'bot.polling' ki jagah lega)
    # Yeh `main` loop ko chalu rakhega
    await web_server()
