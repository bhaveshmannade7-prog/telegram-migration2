...
# Init Bots
# Default parse_mode='Markdown' hi rahega
bot = AsyncTeleBot(BOT_TOKEN, parse_mode='Markdown')
# Client ko yahan define kiya hai, lekin start 'main' mein karenge
...
async def whoami_cmd(message):
    ...
    try:
        # FIX: 'async with app:' hata diya gaya hai.
        # 'app' ab 'main()' mein start ho chuka hai.
        me = await app.get_me() 
        ...
        # Success message hai, 'Markdown' (default) safe hai
...
# Yeh @app.on_message handlers pehle se chalu 'app' client ko use karenge
...
# =================================================================
# === 'Client already connected' WALA FIX YAHAN HAI ===
# =================================================================
async def run_the_index_job():
    if db_pool is None:
        return "❌ DB Not Connected", -1 
    ...
    async with batch_job_lock:
        ...
        for r in rows:
            try:
                ...
                pass # Ignore errors on deleted messages
...
async def main():
    ...
    # 2. Pyrogram Client (app) ko start karo (SIRF EK BAAR YAHAN)
    ...
    if webhook_set:
        ...
        # Bot ko stop kar do agar webhook fail ho
        await app.stop()
        exit(1)
...
if __name__ == "__main__":
    try:
        ...
        print("\nBot stopping...")
        # Bot stop hone par Pyrogram client ko bhi stop karna zaroori hai
