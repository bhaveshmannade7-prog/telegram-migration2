...
async def setup_database():
    ...
    async with app.pool.acquire() as conn:
        ...
        ''')
        # Target duplicates table (Pichhle jaisa)
        await conn.execute('''
        ...
        ''')
        # Caption lock table (Pichhle jaisa)
        await conn.execute('''
...
async def resolve_chat_id(client: Client, ref: str | int, client_name: str):
    """
    ID/Username ko resolve karta hai, (Bot ya User) client ka naam batata hai.
    """
    try:
        chat = await client.get_chat(ref)
        return chat
    except PeerIdInvalid:
         raise RuntimeError(f"❌ Peer not mila. Check karein ki aapka **{client_name}** (`{client.me.username or 'String'}`) uss chat (`{ref}`) mein member hai.")
...
async def scan_channel_history(client: Client, chat, status_message, scan_function):
    """
    User Client 'get_chat_history' ka istemaal karke scan karta hai.
    """
    total_processed = 0
    newly_found = 0
...
# Yeh commands waise hi rahenge, kyunki Bot (Token) ko Target Channel mein Admin hona chahiye
...
async def main():
    ...
    print(f"User {app.user_client.me.first_name} (String) active hai.")
    print("Bot Client (Token) ko start kar raha hoon...")
    # app_bot.start() ko app.run() call kar dega
