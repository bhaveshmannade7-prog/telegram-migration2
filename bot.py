...
from pyrogram.errors import FloodWait, ChatAdminRequired, PeerIdInvalid, RPCError, UserNotParticipant
...
async def resolve_chat_id(client: Client, ref: str | int, client_name: str):
    try:
        ...
        return chat
         ...
         raise RuntimeError(f"❌ Peer not mila. Check karein ki aapka **{client_name}** (`{client.me.username or client.me.first_name}`) uss chat (`{ref}`) mein member hai.")
...
async def start_cmd(_, message):
    await message.reply(
        ...
        "* `/sync_source <chat_id>` - Puraane channel ko poora scan karke database mein daalta hai. (Yeh puraana data delete kar dega)\n"
...
async def start_forward_cmd(client, message):
    ...
    for movie in movies_to_forward:
        ...
        try:
            ...
            # --- FIX: Forwarding ka kaam ab 'app_user' (String) kar raha hai ---
            ...
            await asyncio.sleep(0.5) # User account ke liye thoda pause zaroori hai
...
async def batch_editor(client, message, chat_ref, edit_function):
    global is_editing
    try:
        # Caption edit karne ke liye Bot Client ka istemaal
...
async def main():
    ...
    # FIX: User ka first_name check karo, agar 'None' hai (jaise aapne bataya 'Null') toh fallback do
    ...
    # app.start() ko app.run() call kar dega
    # FIX: Yahaan se `await asyncio.Event().wait()` hata diya hai,
    # kyunki `app.run()` khud hi wait karta hai. Yahi deadlock tha.

if __name__ == "__main__":
    print("Bot ke main event loop ko start kar raha hoon...")
    app.run(main()) # app yaani app_bot ko run karo, jo 'main' ko pehle chalaayega
