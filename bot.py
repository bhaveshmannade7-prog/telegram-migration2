...
async def start_cmd(_, message):
    await message.reply(
        ...
        "3. `/sync_bot_account` - **(Naya)** Bot Account ki memory sync karein.\n\n"
        "**Roz Ka Kaam:**\n"
...
async def sync_bot_account_cmd(client, message):
    ...
    try:
        ...
        await status.edit(f"❌ Bot Sync Error: {e}")
# --------------------
...
async def start_forward_cmd(client, message):
    ...
    for unique_id, data in source_movies.items():
        ...
        except RPCError as e: # Yahaan galti thi, RPCError ko handle karna zaroori hai
            print(f"[FWD RPC ERR] Skipping msg {message_id}: {e}")
            if "USER_IS_BLOCKED" in str(e): # Agar bot block ho jaaye
                is_forwarding = False
