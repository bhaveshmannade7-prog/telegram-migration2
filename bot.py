...
async def start_command(message):
    ...
    if user_id == ADMIN_ID:
        ...
        await bot.reply_to(
            ...
            "**ज़रूरी:** काम करने के लिए मुझे सोर्स और बैकअप दोनों चैनलों में एडमिन बनाएँ।\n\n"
            "**बॉट अब फुली ऑटोमैटिक है:**\n"
            "1.  **ऑटो-क्लीन:** सोर्स चैनल में नई पोस्ट के कैप्शन अपने आप साफ़ हो जाएँगे।\n"
            "2.  **ऑटो-फुटर:** साफ़ कैप्शन में आपका फुटर अपने आप जुड़ जाएगा।\n"
            "3.  **ऑटो-डिलीट डुप्लीकेट:** अगर कोई मूवी (वीडियो/डॉक्यूमेंट) पहले से ही डेटाबेस में है, तो नई पोस्ट अपने आप डिलीट हो जाएगी।\n" # <--- फिक्स यहाँ है
            "4.  **ऑटो-ब्लैकलिस्ट:** अगर कैप्शन में कोई ब्लैकलिस्टेड शब्द (जैसे 18+) मिलता है, तो पोस्ट अपने आप डिलीट हो जाएगी।\n\n"
            "नीचे दिए गए बटन से आप आँकड़े देख सकते हैं:",
...
async def handle_new_movie_post(message):
    ...
    print(f"📥 सोर्स चैनल में नया मैसेज मिला: {message.message_id}")
    
    if not db_pool:
        print("⚠️ नया पोस्ट मिला, लेकिन DB कनेक्टेड नहीं है। क्लीनिंग और डुप्लीकेट चेक स्किप किया जा रहा है।")
        return

    try:
        file_unique_id = get_file_unique_id(message)
        if not file_unique_id:
            print("ℹ️ पोस्ट में कोई वीडियो/डॉक्यूमेंट नहीं है। स्किप किया जा रहा है।")
            return
        ...
        for word in BLACKLIST_WORDS:
            if word in caption_lower:
                print(f"🚫 ब्लैकलिस्टेड शब्द '{word}' मिला। मैसेज {message.message_id} डिलीट किया जा रहा है।")
                try:
                    await bot.delete_message(message.chat.id, message.message_id)
                except ApiTelegramException as e:
                    print(f"❌ मैसेज डिलीट करने में विफल (ब्लैकलिस्ट): {e}")
        ...
        async with db_pool.acquire() as conn:
            ...
            if is_duplicate:
                print(f"🚫 डुप्लीकेट मूवी मिली! मैसेज {message.message_id} डिलीट किया जा रहा है।")
                try:
                    await bot.delete_message(message.chat.id, message.message_id)
                except ApiTelegramException as e:
                    print(f"❌ मैसेज डिलीट करने में विफल (डुप्लीकेट): {e}")
                return # आगे कुछ न करें
        ...
        if final_caption.strip() == original_caption.strip():
            ...
            print(f"ℹ️ मैसेज {message.message_id} का कैप्शन पहले से ही साफ़ है।")
        ...
        if caption_changed:
            try:
                ...
                print(f"✅ मैसेज {message.message_id} का कैप्शन साफ़ किया गया और फुटर जोड़ा गया।")
            except ApiTelegramException as e:
                if "message is not modified" in str(e):
                    print(f"ℹ️ मैसेज {message.message_id} का कैप्शन पहले से ही साफ़ है (API Error)।")
                else:
                    print(f"❌ कैप्शन एडिट करने में विफल: {e}")
                    # अगर कैप्शन बहुत लंबा है या कोई और एरर है, तब भी हम इसे DB में जोड़ देंगे
                    pass
        ...
        print(f"💾 नई मूवी (Msg ID: {message.message_id}) डेटाबेस में सहेजी गई।")

    except Exception as e:
        print(f"‼️ चैनल पोस्ट हैंडलर में गंभीर त्रुटि: {e}")
        import traceback
        traceback.print_exc()
...
async def handle_edited_movie_post(message):
    ...
    print(f"🔄 मैसेज {message.message_id} एडिट हुआ। पुनः जाँच की जा रही है...")
