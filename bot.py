# 'keep_alive' ko import karo, lekin 'run' function ko alag se thread me chalayenge
from keep_alive import keep_alive 
from threading import Thread # Naya: keep_alive ko thread me chalane ke liye
...
# FIX: `no_updates=True` lagaya taaki yeh background me crash na ho
app_user = Client(
    "user_session",
    api_id=API_ID,
    api_hash=API_HASH,
    session_string=SESSION_STRING,
    no_updates=True # YEH SABSE ZAROORI FIX HAI
)
...
async def setup_database():
    ...
    # --- NAYA DEBUGGING STEP ---
    # Hum URL ka aakhri 25 character print karenge yeh check karne ke liye ki woh 'internal' hai ya nahi
    if DATABASE_URL:
        print(f"DEBUG: Istemaal ho raha DB URL (aakhri 25 char): ...{DATABASE_URL[-25:]}")
    # ---------------------------

    try:
        # --- SUDHAAR (FIX): Timeout ko 60 SECOND kiya ---
        # Free DBs ko jaagne me 30-60 sec lag sakte hain.
...
async def start_forward_cmd(client, message):
    ...
    for movie in movies_to_forward:
        ...
        except (MessageIdInvalid, MessageAuthorRequired): # <--- YEH ERRORS HANDLE KARNA ZAROORI HAI
...
async def replace_caption_cmd(client, message):
    try:
        ...
        if chat_ref.isdigit(): chat_ref = int(chat__ref)
    except IndexError:
...
async def main():
    ...
    print("--- Bot Ab Poori Tarah Active Hai ---")
